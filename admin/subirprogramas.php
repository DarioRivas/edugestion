...
if (isset($_POST['cargar'])) {
  ...
  $path = "../docentes/programas/"; //ruta donde se almacena el archivo              
  date_default_timezone_set('America/Argentina/Jujuy');
  $date = new DateTime();
  $timestamp = $date->format('Y-m-d H:i:s');
  $aniolectivo = date('Y');
  list($docente, $apellido) = explode('|', $_POST['docente']);
  $cargadopor = $apellido;
  $cargadoporNombre = $docente;
...
?>
<main label="subirprogramasadmin">
  <header>
  ...
  <section>
    <header>
      <div class="text-center mb-3">
        <h3>
          Subir programas anuales al sistema
        </h3>
      </div>
    </header>
  </section>
          ...
          <div class="col-xl-4 col-12 mt-3">
            ...
            <select class="form-select" name="docente" id="docente" required>
              ...
              while ($usuario = mysqli_fetch_array($get_usuarios)) { ?>
                <option value="<?= $usuario['apellido'] . ' ' . $usuario['nombre'] ?>|<?=$usuario['apellido']?>">
