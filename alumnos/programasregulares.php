  ...
  <section class="container-fluid">
    <div class="card mt-3 shadow small ">
      <div class="card-header text-bg-warning d-flex align-items-center">
        ...
        PROGRAMAS REGULARES DE <?= $anio ?>º AÑO
      </div>
      <div class="card-body">
        <?php
        $materias = get_Materias($anio);
        while ($materia = mysqli_fetch_array($materias)) { ?>
          <?php $trabajos = get_ProgramasRegularesAnio($materia['nomenclatura']);
          $rad = $flag % 2;
          if ($rad == 0) {
            ...
            <?php if (mysqli_num_rows($trabajos) != 0) {
              while ($trabajo = mysqli_fetch_array($trabajos)) {
                ...
                <div class="col-12 col-xl-4 my-1 d-flex align-items-center">
                  <?php if ($trabajo['division'] == '0') {
                    $extras = '';
                  } else {
                    $extras = ' (' . $anio . '° ' . $trabajo['division'] . 'a Turno ' . $trabajo['turno'] . ')';
                  } ?>
                  <i class='bx bxs-circle me-2'></i> <?= $materia['nombre'] . $extras ?>
                </div>
                ...
                <div class="col-12 col-xl-4 my-1 d-flex align-items-center">
                  <a href="../docentes/programasregulares/<?= $trabajo['archivo'] ?>" target="_blank" class="btn btn-sm btn-success d-flex align-items-center w-100"> <i class='bx bxs-file-pdf text-white bx-sm me-2'></i> Descargar programa
